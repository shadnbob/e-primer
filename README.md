...
- **Logical Fallacies** — the named fallacy catalog (8 subcategories: red herring & non sequitur, begging the question, appeals & bandwagon, cherry-picking & selection, goalposts & burden & No True Scotsman, post hoc & causal shortcuts, false equivalence & extreme comparisons, the fallacy fallacy & question games). Every card runs in both directions: the fallacy is a real reasoning failure, and the accusation is a claim that needs the same evidence it demands. The capstone is the fallacy fallacy — naming a fallacy discounts an argument, never the conclusion. Statistical "post hoc analysis" and finance "sunk costs" never trigger.
...
1. **Pattern compilation** — all word lists and regex patterns from the 23 dictionaries are pre-compiled into optimized RegExp objects
