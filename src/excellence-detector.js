// Excellence Detection Module - Can be integrated into content.js
// This module adds positive writing pattern detection to complement bias detection
...
class ExcellenceDetector {
    ...
    calculateIntensity(text, type) {
        ...
        for (let level = 3; level >= 1; level--) {
            if (levels[level].some(word => lowerText.includes(word))) {
                return level;
            }
        }
        return 2;
    ...
    createHoverCardContent(match, nearbyMatches = []) {
        ...
        if (match.isExcellence) {
            content += `
                <div class="hover-card-header">✓ ${this.excellencePatterns[match.type].name}</div>
                <div class="hover-card-text">"${match.text}"</div>
                <div class="hover-card-reason">${match.tooltip}</div>
            `;
        } else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ExcellenceDetector;
}
