...
- **Settings persistence**: All settings reads/writes go through `src/utils/settings-storage.js` (dual-write to `storage.sync` + `storage.local`, reads merge local over sync over defaults, `lastError` checked and logged). Never call `chrome.storage.sync.set` directly for settings, and never write the full settings snapshot — save only the changed keys (a stale full-object write clobbers keys changed elsewhere; a silently failing sync write made popup toggles appear to never persist)
